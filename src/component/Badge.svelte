<template lang="pug">
    .svBadge(bind:this="{elm}",class:svBadge--visible="{isShown}",class="{styleType}")
        .svBadge__icon(class="{iconStyle}")
        .svBadge__content
            .svBadge__content_title
                .svBadge__content_title_text {title}
                .svBadge__content_title_close(class:svBadge__content_title_close--visible="{(close === 'click')}",on:click="{hide}")
                    i.icon-cancel
            .svBadge__content_text {text}
</template>

<script>
    // TODO Dialog, Badge and Notifaction have a lot in common, create sort of abstract base component to extend from
    // TODO allow to define content for badge that isn't just text alone. it should support full HTML, but then
    //  we need to have a simple way to define it via slot or similar
    export let type = undefined;
    export let close = undefined;

    let elm;
    let title   = 'N/A';
    let text    = 'N/A';
    let icon    = 'cancel';
    let isShown = false;
    let iconMap = {
        'error'   : 'cancel',
        'success' : 'check',
        'info'    : 'info',
        'question': 'help'
    };
    let timer;

    $: iconStyle = type && iconMap[type] ? 'icon-' + iconMap[type] : '';
    $: styleType = type ? 'svBadge--' + type :'';

    /**
     * This is a listener for the global event, as theoretically there
     *  should only be one instance of Badge. Practically there could be
     *  many, they might all be triggered then by this.
     */
    window.eventBus.bind('sv-badge', e => {
        title = e.detail.title || 'N/A';
        text = e.detail.text || 'N/A';
        type = e.detail.type || undefined;
        isShown = true;

        if (close !== 'click') {
            let time = /([\d\.]+)(s|ms)/.exec(close);

            if (time && time[1] && time[2]) {
                if (timer) {
                    clearTimeout(timer);
                }
                timer = setTimeout(() => isShown = false, Math.ceil(
                    parseFloat(time[1]) * (time[2] === 's' ? 1000 : 1)));
            }
        }
    });

    /**
     * This defines a global method to trigger a badge, as there should be only
     *  one badge that is filled with information for the time being.
     */
    window.badge = {};
    window.badge.show = e => {
        if (e.target){
            window.eventBus.dispatch('sv-badge', {
                title: e.target.dataset.title,
                text : e.target.dataset.text,
                type : e.target.dataset.type});
        }
    };

    function hide () {
        isShown = false;
    }
</script>

<style lang="scss" global>
    @import '../App';

    .svBadge {
        background-color: transparent;
        color: white; // TODO do we need to define a specific text-color in the scheme. or different scheme colors for used badge-color-variations? -> var(--fg-color);
        width: 100%;
        height: 0;
        margin: 0;
        opacity: 0;
        transition: height 250ms, opacity 250ms;
        overflow: hidden;
        display: flex;

        &__icon {
            flex-grow: 0;
            font-size: 3em;
            min-width: 1em;
            padding-top: .25em;
        }

        &__content {
            flex-grow: 1;
            display: flex;
            flex-direction: column;

            &_title {
                flex-grow: 0;
                padding-left: .5em;
                padding-right: 1em;
                font-size: 1.25em;
                font-weight: bolder;
                display: flex;

                &_text {
                    flex-grow: 1;
                }

                &_close {
                    flex-grow: 0;
                    display: none;

                    &--visible {
                        display: inline-block;
                    }
                }
            }

            &_text {
                flex-grow: 1;
                padding-left: 1em;
            }
        }

        &--visible {
            height: 80px;
            opacity: 1;
        }

        // TODO use x.js/shaping for this
        &--info {
            background-color: var(--primary-color-darker);
            border-top: 1px solid var(--primary-color);
            border-bottom: 1px solid var(--primary-color);
        }

        &--question {
            background-color: var(--secondary-color-darker);
            border-top: 1px solid var(--secondary-color);
            border-bottom: 1px solid var(--secondary-color);
        }

        &--error {
            background-color: var(--negative-color-darker);
            border-top: 1px solid var(--negative-color);
            border-bottom: 1px solid var(--negative-color);
        }

        &--success {
            background-color: var(--positive-color-darker);
            border-top: 1px solid var(--positive-color);
            border-bottom: 1px solid var(--positive-color);
        }
    }
</style>
